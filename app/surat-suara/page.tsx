...
export default function SuratSuaraPage() {
  ...
  const [selectedCandidate, setSelectedCandidate] = useState<typeof DUMMY_CANDIDATES[0] | null>(null);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [showSuccessToast, setShowSuccessToast] = useState(false);
  const [flippedId, setFlippedId] = useState<number | null>(null);

  const handleVoteClick = (candidate: typeof DUMMY_CANDIDATES[0]) => {
  ...
  const handleConfirmVote = () => {
    ...
    // Simulasi Proses Jaringan / API Call selama 2 detik
    setTimeout(() => {
      setIsSubmitting(false);
      setSelectedCandidate(null);
      setShowSuccessToast(true);
      
      // Tunggu toast terbaca selama 2.5 detik, lalu redirect ke Beranda
      setTimeout(() => {
        router.push('/');
      }, 2500);
    }, 2000);
    ...
    <div className="min-h-screen bg-slate-50/80 py-8 sm:py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 lg:gap-16 perspective-[2000px]">
          {DUMMY_CANDIDATES.map((paslon, index) => (
            // Wrapper dengan persistent glow merah di balik card
            <div key={paslon.id} className="relative group">
              {/* Glow orb merah — selalu ada, makin terang saat hover */}
              <div
                aria-hidden="true"
              ...
              >
                ...
                <div className="absolute inset-0 [backface-visibility:hidden] bg-white rounded-3xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.1)] overflow-hidden flex flex-col group border border-gray-100">
                  <div className="relative h-3/5 bg-gray-200 overflow-hidden">
                    <img src={paslon.image} alt={`Paslon ${paslon.noUrut}`} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                    ...
                    <div className="absolute top-6 right-6 bg-merah-formal/90 backdrop-blur-md px-5 py-1.5 rounded-full shadow-lg border border-red-400/30">
                      <span className="font-bold text-white tracking-widest text-sm">PASLON {paslon.noUrut}</span>
                    </div>

                    {/* Informasi Nama Overlay */}
                    <div className="absolute bottom-6 left-8 right-8 text-white">
                      <p className="text-[10px] font-bold text-red-300 uppercase tracking-widest mb-1 opacity-90">Calon Ketua BEM</p>
                      <h2 className="text-3xl font-serif font-bold leading-tight shadow-sm">{paslon.presiden}</h2>
                      <p className="text-xs font-mono text-gray-300 opacity-80 mb-3 mt-1">NPM: {paslon.npmPresiden}</p>
                      ...
                      <h2 className="text-2xl font-serif font-bold leading-tight shadow-sm">{paslon.wapres}</h2>
                      <p className="text-xs font-mono text-gray-300 opacity-80 mt-1">NPM: {paslon.npmWapres}</p>
                    </div>
                  </div>
                  
                  <div className="flex-grow p-8 flex flex-col justify-center gap-4 bg-white relative">
                    <div className="absolute -top-4 left-1/2 -translate-x-1/2 w-8 h-8 bg-white rotate-45"></div> {/* Decorative arrow */}
                ...
                >
                  <div className="flex justify-between items-center px-8 py-6 border-b border-gray-100 bg-white/90 backdrop-blur-md">
                    <h3 className="text-2xl font-serif font-bold text-gray-900 flex items-center gap-3">
                      <span className="w-10 h-10 rounded-full bg-merah-formal flex items-center justify-center text-white text-lg">{paslon.noUrut}</span>
                      Visi & Misi
                    </h3>
                    <button 
                  ...
                  <div className="p-6 border-t border-gray-100 bg-white">
                    ...
                    >
                      SAYA YAKIN, COBLOS PASLON {paslon.noUrut}
                    </button>
                  </div>
                </div>
              </motion.div>
      ...
      {/* Backdrop & Modal Dialog Konfirmasi */}
      <AnimatePresence>
        {selectedCandidate && (
          <motion.div 
            initial={{ opacity: 0 }}
            ...
            >
              ...
              <div className="p-8 text-center relative">
                <div className="w-24 h-24 bg-red-50 rounded-full flex items-center justify-center mx-auto mb-6 shadow-inner border-4 border-white -mt-16 relative z-20">
                  <span className="text-4xl font-bold text-merah-formal">{selectedCandidate.noUrut}</span>
                </div>
                
                <p className="text-lg text-gray-700 mb-2 leading-relaxed">
                  Apakah Anda yakin memberikan suara untuk <br/> <strong className="text-2xl text-gray-900 font-serif block mt-2">Paslon Nomor {selectedCandidate.noUrut}?</strong>
                </p>
