    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
      ...
      {loading ? (
        <div className="py-20 text-center text-slate-500">
          ...
          <p className="font-medium text-sm">Memuat Pasangan Calon dari Supabase...</p>
        </div>
      ) : candidates.length === 0 ? (
        <div className="py-16 text-center text-slate-500 bg-white rounded-2xl border border-slate-200 p-8 max-w-md mx-auto">
          <User className="w-12 h-12 text-slate-300 mx-auto mb-3" />
          <p className="font-semibold text-slate-700">Belum Ada Pasangan Calon Aktif</p>
          <p className="text-xs text-slate-400 mt-1">Paslon akan ditampilkan setelah diverifikasi oleh Panitia KPU.</p>
        </div>
      ) : (
          ...
          {candidates.map((paslon, index) => (
            ...
            >
              ...
              <div className="absolute top-6 right-6 bg-slate-900 backdrop-blur-md px-4 py-1.5 rounded-md shadow-lg z-20 border border-slate-700">
                <span className="font-bold text-amber-400 tracking-widest text-xs">PASLON {paslon.number}</span>
              </div>
              ...
              <div className="relative h-72 bg-red-900/5 overflow-hidden group flex items-center justify-center">
                ...
                <div className="absolute bottom-6 left-8 text-white z-20 pr-8">
                  <h2 className="text-3xl font-black font-serif mb-1 drop-shadow-lg leading-snug">
                    {paslon.chairman} <br/><span className="text-gray-300 text-2xl font-normal">&amp;</span> {paslon.vice_chairman}
                  </h2>
